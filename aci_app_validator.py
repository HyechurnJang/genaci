...
class Validator(object):
    def __init__(self):
        ...
        self.signature = ''
        self.UT = False
        self.metaversion = '1.0'    # Metaversion actually used for app validation
        self.curmetaversion = '1.1' # Current validator metaversion used by packager and tools
        self.minmetaversion = '1.0' # Minimum metaversion for external appcenter validation
    ...
    def getResult(self):
        if (self.err_code != 0):
            result = [{ 'status' : 'failed'}, {'errCode' : self.err_code, 'errMsg' : self.err_msg, 'errType' : self.err_type}]
        else:
    ...
    def exception_type(self,callType,err_code,err_msg):
        ...
        elif callType == 'apic' or callType == 'generator':
            pass
        else:
            raise Exception()
    ...
    def validateMandatoryFields(self,filename,callType,appState):
        ...
        if callType!="apic":
            if 'contact' not in data:
                self.err_code = 22
                self.err_msg = "Contact field missing in JSON file"
                logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                return self.err_code
        ...
        if appState == "stateless":
            if 'api' in data:
                ...
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        global apiFieldPresent
        apiFieldPresent=True
        return 0
    ...
    def get_appmetaversion(self,appFolder):
        os.chdir(appFolder)
        metaversion = ''
        if os.path.isfile('app.json') == True:
            data = json.load(open('app.json', 'r'))
            if 'metaversion' in data.keys():
                metaversion=data['metaversion']
            else:
                metaversion="1.0"
        return metaversion

    def get_curmetaversion(self):
        return self.curmetaversion

    def get_minmetaversion(self):
        return self.minmetaversion

    def set_metaversionforvalidation(self, metaversion):
        self.metaversion = metaversion
        logger.info('Metaversion for validation set to {0}'.format(self.metaversion))
        ...
        if appState == "stateful":
            ...
            if os.path.exists('Image') == True:
                ...
                if len(files) > 1:
                    ...
                    self.exception_type(callType,self.err_code,self.err_msg)
                    return self.err_code, self.err_msg
                if callType != 'apic' and float(self.metaversion) >= 1.1:
                    try:
                        if zipfile.is_zipfile(files[0]):
                            cmd = 'mkdir -p tmp/; unzip %s -q -d tmp/' % (files[0])
                        else:
                            cmd = 'mkdir -p tmp/; tar xfz %s -C tmp/' % (files[0])
                            logger.info('Extracting the contents of %s into %s' %(files[0],os.getcwd()+"/tmp/"))
                        os.system(cmd)
                    except:
                        self.err_code = 109
                        self.err_msg ="Unable to extract the *t*gz file "
                        logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
                    logger.info('Extraction successful')
                    logger.info('Checking if required file exists')
                    if os.path.isfile(os.getcwd()+"/tmp/repositories")==True:
                        logger.info("Required file %s  exists" %(os.getcwd()+"/tmp/repositories"))
                    else:
                        logger.info("Required file %s does not exists" %(os.getcwd()+"/tmp/repositories"))
                        logger.info("Checking if alternate required file %s  exists" %(os.getcwd()+"/tmp/manifest.json"))
                        if os.path.isfile(os.getcwd()+"/tmp/manifest.json")==True:
                            logger.info("Required file %s  exists" %(os.getcwd()+"/tmp/manifest.json"))
                        else:
                            self.err_code = 109
                            self.err_msg =" Required file inside the Image directory "+os.getcwd()+"/tmp/repositories  or   "+os.getcwd()+"/tmp/manifest.json  does not exist"
                            logger.error('Error occured: %d %s' %(self.err_code, self.err_msg))
                            self.exception_type(callType,self.err_code,self.err_msg)
                            return self.err_code, self.err_msg
                    logger.info('Removing the directory %s' %(os.getcwd()+"/tmp/"))
                    cmd="rm -rf tmp "
                    os.system(cmd)
            else:
                self.err_code = 110
        ...
        if os.path.exists('Media') == True:
            ...
            if callType != 'apic':
                if os.path.exists('./Readme') == True:
                    ...
                    if(len(readme)>1):
                        ...
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
                    if float(self.metaversion) >= 1.1:
                        if(len(readme)>0):
                            fileType=magic.from_file(readme[0])
                            if 'ASCII' not in fileType and 'UTF-8' not in fileType:
                                self.err_code = 114
            ...
            if callType != 'apic':
                if os.path.exists('./ReleaseNotes') == True:
                    ...
                    if float(self.metaversion) >= 1.1 and len(releasenotes)>0:
                ...
                if os.path.exists('./License') == True:
                    ...
                    if float(self.metaversion) >= 1.1:
                        for i in license:
                            ...
                            if 'ASCII' not in fileType and 'UTF-8' not in fileType:
                                self.err_code = 126
                                self.err_msg =i+" is not valid .txt file, the file needs to have some ASCII content"
                                logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                                self.exception_type(callType,self.err_code,self.err_msg)
                                return self.err_code, self.err_msg
                else:
            ...
            if callType != 'apic':
                if (os.path.exists('./IntroVideo') == True):
                    ...
                    if len(videoFile)==0:
                        self.err_code = 120
                        self.err_msg ="A video file of type .mp4 should be present inside IntroVideo directory"
                        logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
                    if len(videoFile)>1:
                    ...
                    if float(self.metaversion) >= 1.1:
                        self.err_code = 121
                        self.err_msg ="Required IntroVideo directory not found inside /Media"
                        logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
            os.chdir(extractLoc)
            ...
            if callType != 'apic':
                if os.path.exists('./Snapshots') == True:
                    ...
                    if(len(allFiles) == 0):
                        self.err_code = 123
                        self.err_msg ="Snapshots should contain atleast 1 file of type .jpg or .gif or .png"
                        logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
                    if(len(allFiles) != len(imageFiles)):
                        ...
                        for i in imageFiles:
                            fileType=magic.from_file(i)
                            if 'PNG image data' not in fileType and 'GIF image data' not in fileType and 'JPEG image data' not in fileType:
                                self.err_code = 124
                    ...
                    if float(self.metaversion) >= 1.1:
                        self.err_code = 123
                        self.err_msg ="Required Snapshots directory inside /Media"
                        logger.error('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code, self.err_msg
        else:
    ...
    def validateAppApis(self,callType,appState):
        ...
        for key,value in self.appMeta["api"].items():
                ...
                if size_key<=0 or size_key>256:
                    ...
                    self.exception_type(callType,self.err_code,self.err_msg)
                    return self.err_code
                split1=str(key).split("/")
                count=0
                for i in split1:
                    count=count+1
                if(count>4):
                    self.err_code = 153
                    self.err_msg = "api- "+key+"  is not supported"
                    logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                    self.exception_type(callType,self.err_code,self.err_msg)
                    return self.err_code
                size=len(key)
                last5char=key[-5:size]
                last4char=key[-4:size]
                if(last5char!=".json"):
                    if(last4char!=".xml"):
                        self.err_code = 154
                        self.err_msg = "api- "+key+"  api should be of type .json or .xml"
                        logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                        self.exception_type(callType,self.err_code,self.err_msg)
                        return self.err_code
                value=value.strip()
    ...
    def validateCategory(self,callType):
        ...
        allowed_category=["Beta","Tools and Utilities","Visibility and Monitoring","Optimization","Security","Networking","Cisco Automation and Orchestration"]
    ...
    def validateAppContactEmail(self,callType):
        logger.info('Validating contact email field')
        email=''
        try:
            ...
            if len(email)==0:
                self.err_code = 22
                self.err_msg = "contact-email field missing inside contact in JSON file"
                logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        ...
        if(len(email)>0):
            ...
            if(str(emailValidationResult) != "True"):
                ...
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        logger.info('Contact field email validation successful')
        return 0

    def validateAppContactPhone(self,callType):
        if float(self.metaversion) < 1.1:
            return 0

        logger.info('Validating contact phone')
        phone=''
        try:
            ...
            if len(phone)==0:
                self.err_code = 22
                self.err_msg = "contact-phone field missing inside contact in JSON file"
                logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        ...
        if len(phone)==0:
            self.err_code = 22
            self.err_msg = "contact-phone cannot be empty, please provide a valid phone number"
            logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
            self.exception_type(callType,self.err_code,self.err_msg)
            return self.err_code

        num=phone.strip('+').replace(' ','').replace('-','')

        if not num.isdigit():
            self.err_code = 22
            self.err_msg = "Invalid contact-phone, it cannot only have digits, space, dash and + sign"
            logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
            self.exception_type(callType,self.err_code,self.err_msg)
            return self.err_code
        ...
        if len(num)<10:
            self.err_code = 22
            self.err_msg = "Invalid contact-phone, please provide 10 digit phone number"
            logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
            self.exception_type(callType,self.err_code,self.err_msg)
            return self.err_code

        logger.info('Contact phone field validation successful')
        return 0

    def validateAppContactUrl(self,callType):
        logger.info('Validating contact url')
        url=''
        try:
            ...
            if len(url)==0:
                self.err_code = 22
                self.err_msg = "contact-url field missing inside contact in JSON file"
                logger.info('Error occurred: %d %s' %(self.err_code, self.err_msg))
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        # Validate contact-url
        url=url.strip()
        if len(url) > 0 and float(self.metaversion) < 1.1:
            ...
            if(str(urlValidationResult) != "True"):
                ...
                self.exception_type(callType,self.err_code,self.err_msg)
                return self.err_code
        logger.info('Contact URL field validation successful')
        return 0
    ...
    def getFileType(self,type):
        if type==1:
            ...
            correctlast4characters=".png/.jpg/.gif"
        elif type==3:
    ...
    def getFileNameSize1(self,mediaType,type,callType):
        ...
        if(len(correctlast4characters)>4):
            ...
            if((split_correctchar[0] != retrievedLast4Characters) and (split_correctchar[1] != retrievedLast4Characters)  and (split_correctchar[2] != retrievedLast4Characters)):
                self.err_code = 192
    ...
    def getSize(self,size,media,type,callType):
        if size==1:
            temp=self.getFileNameSize1(media,type,callType)
            return temp
        if size==0:
            return 0
        if size>1:
            temp=self.getFileNameSize2(media,type,size,callType)
            return temp
    ...
    def validateMedia(self,callType):
        ...
        temp=self.getSize(s1,self.appMeta["media"]["introvideo"],3,callType)
        if(temp!=0):
            return temp
        else:
    ...
    def validateAppMetaData(self, callType, appState):
        ...
        if self.validateAPICVersion(callType) != 0:
            return self.err_code, self.err_msg
        if callType != "apic":
            if self.validateAppContactEmail(callType) != 0:
                return self.err_code, self.err_msg
            if self.validateAppContactPhone(callType) != 0:
                return self.err_code, self.err_msg
            if self.validateAppContactUrl(callType) != 0:
                return self.err_code, self.err_msg
    ...
    def main(self, filename, sourceDir, outputDir, publicKeyPath):
        try:
            ...
            metaversion=self.get_appmetaversion(appFolder)
            logger.info('App metaversion is: {0}'.format(metaversion))
            if metaversion == '':
                self.err_code = 102
                self.err_msg ="Unable to get app metaversion"
                raise Exception()

            if float(metaversion) < float(self.minmetaversion):
                self.err_code = 102
                self.err_msg ="App metaversion is less that minimum supported metaversion ({0}). Please repackage the app with latest tools.".format(self.minmetaversion)
                raise Exception()

            self.set_metaversionforvalidation(metaversion)
    ...
    @staticmethod
    def validateJsonFieldsDuringGeneration(json_dict, state):
        global logger
        callType = 'generator'
        validator = Validator()
        validator.appMeta = json_dict
        validator.UT = True
        metaversion=validator.get_curmetaversion()
        validator.set_metaversionforvalidation(metaversion)
        validator.setLogger("/tmp/aci_app_validator.log", "apic")

        if 'author' in json_dict and validator.validateAuthor(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'category' in json_dict and validator.validateCategory(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'appid' in json_dict and validator.validateAppId(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'version' in json_dict and validator.validateAppVersion(str(validator.appMeta["version"]),'app',callType) != 0:
            return validator.err_code, validator.err_msg
        if 'iconfile' in json_dict and validator.validateIconFile(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'name' in json_dict and validator.validateAppName(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'shortdescr' in json_dict and validator.validateShortDescription(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'vendor' in json_dict and validator.validateAppVendor(callType) != 0:
            return validator.err_code, validator.err_msg
        if 'apicversion' in json_dict and validator.validateAPICVersion(callType) != 0:
            return validator.err_code, validator.err_msg
        if state == "stateful" and 'api' in json_dict and validator.validateAppApis(callType,state) != 0:
            return validator.err_code, validator.err_msg
        if 'permissions' in json_dict and validator.validateAppPermissions(callType,state) != 0:
            return validator.err_code, validator.err_msg
        if 'vendordomain' in json_dict and validator.validateVendorDomain(callType) != 0:
            return validator.err_code, validator.err_msg

        # Tweak value if no given to have a more "explicit" error message for the creator
        if 'contact' in json_dict and 'contact-email' in json_dict['contact']:
            if len(validator.appMeta['contact']['contact-email']) <= 0:
                validator.appMeta['contact']['contact-email'] = '-1'
            if validator.validateAppContactEmail(callType) != 0:
                return validator.err_code, validator.err_msg
        if 'contact' in json_dict and 'contact-phone' in json_dict['contact']:
            if len(validator.appMeta['contact']['contact-phone']) <= 0:
                validator.appMeta['contact']['contact-phone'] = '-1'
            if validator.validateAppContactPhone(callType) != 0:
                return validator.err_code, validator.err_msg
        if 'contact' in json_dict and 'contact-url' in json_dict['contact']:
            if len(validator.appMeta['contact']['contact-url']) <= 0:
                validator.appMeta['contact']['contact-url'] = '-1'
            if validator.validateAppContactUrl(callType) != 0:
                return validator.err_code, validator.err_msg
